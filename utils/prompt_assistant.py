...
- 这次任务的目标是“帮另一个模型写工作指令”，不是“替这个模型现场做预测分析”，也不是“写一段讲解预测过程的说明文”。
...
10. 最终提示词正文必须是连续自然段或紧凑短句，不要输出编号列表、项目符号、分隔线、标题、前言、后记或“预测过程/输出格式/说明”等小标题。
11. 第二段说明也不要使用编号或项目符号，只用 2-4 句自然语言简短说明。
12. 如果“当前已有提示词”里已经出现编号列表、项目符号、分隔线、小标题、PC28 语义、固定输出示例或明显低质量结构，请把它视为需要被替换的旧版本，只参考少量有价值的信息，不要继承其结构和措辞。
13. 最终提示词正文必须像“系统工作指令”而不是“读者说明书”，不要出现“预测过程：”“请输出以下字段的预测结果：”“下面是规则：”“输出格式如下：”这类元说明句式。
14. 如果需要表达分析步骤，请把它们自然融入指令句中，例如“先结合……再校正……若冲突则……”，而不是展开成显式步骤列表。
15. 如果当前已有提示词只是半成品或说明文风格，你必须把它重构成真正可执行的提示词正文，而不是做轻微润色。

请按以下方式回复：
- 第一段：仅输出可直接粘贴到项目里的最终提示词正文；不要标题、不要编号、不要代码块、不要项目符号、不要分隔线，也不要额外开场。
- 空一行后，第二段：用 2-4 句简短说明你用了哪些变量、为什么这样安排；不要标题、不要编号、不要项目符号。

我当前已有提示词（如果它质量差、结构混乱或不符合上面的要求，请直接重写，不要沿用它的结构）：
{current_prompt or '（暂无）'}
